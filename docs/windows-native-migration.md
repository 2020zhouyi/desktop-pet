...
状态：代码候选已完成；Windows 11 ARM64 来宾中的 x64 单屏早期验收已收口。按本轮决定跳过剩余 VM 操作，登录自启复测、混合 DPI/多屏、真实 x64 和发布切换仍待发布前完成。当前公开 Windows 发布仍以 Electron ZIP 为回滚真相。
...
| 缩放 | 84–228 / 12px 几何测试 | 右下角拖拽与重启恢复 | 直接缩放通过，重启恢复待验 |
| 气泡 | `bubble_contract` | welcome/click/drag/petSwitch | click 可见，完整场景证据待验 |
| picker | `picker_contract` | 搜索、分页、预览、确认、Escape | 搜索/分页/预览/确认通过，Escape 待验 |
...
4. 右下角把手按 12px 档位在 84–228px 缩放，退出重开后恢复。
5. 右键只能打开选择器或退出；托盘可打开选择器、唤醒桌宠和退出。
6. picker 搜索、4×2 分页、真实缩略图、预览/已确认区分、确认切换、Escape 关闭均可用；关闭 picker 不退出桌宠。
7. 切宠后进入 jumping、显示 petSwitch 气泡并在重启后恢复选择。
8. 勾选开机自启后检查 Run 值；注销或重启后日志出现 `source=autostart` 和 `window_visible`，且桌宠仍可拖动。取消勾选后 Run 值消失。
9. 在 100%、125%、150%、200% 缩放下拖到四边；桌宠留在当前工作区内。
10. 副屏置于主屏左侧或上方形成负坐标，并使用混合 DPI；跨屏后尺寸更新、抓取点不漂移、仍可继续拖动。

ARM64 Windows 的 x64 仿真可验证大部分交互，但第 8–10 项在发布前还要用 Windows x64 硬件或等价环境复核。
